...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateParallax() {
        ...
        icons.forEach(icon => {
            const speed = parseFloat(icon.getAttribute('data-parallax'));
            const moveX = currentX * 50 * speed; // Reduced multiplier for subtler movement
            const moveY = currentY * 50 * speed;
            
            // Combine parallax movement with floating animation
            icon.style.transform = `translate(${moveX}px, ${moveY}px) translateY(var(--float-offset, 0))`;
        });

        // Continue animation if movement hasn't settled
    ...
    icons.forEach((icon, index) => {
        ...
        const duration = 2 + Math.random(); // Random duration between 2-3s
...
// course features section start
        // Update the animation script with new class names
      ...
      // Observe all cards with new class name
